...
<%
    ...
    String[] queries = new String[]{
         ...
         */
        "select" + nl
        + " {[Measures].[Transaction Count],[Measures].[Transaction Amount]} on columns," + nl
        + " [Terminal.BySuburb].MEMBERS on rows" + nl
        + "from [ANZ Transaction]",
        "select" + nl
        + " {[Measures].[Transaction Count],[Measures].[Transaction Amount]} on columns," + nl
        + " {crossjoin([Customer.ByGender].[All Genders].Children,[Customer].[All Customers].Children)} on rows" + nl
        + "from [ANZ Transaction]",
        "with MEMBER [Measures].[m] as Median(" + nl
        + "        [Terminal.BySuburb].[All Suburbs].Children," + nl
        + "        [Measures].[Transaction Amount]" + nl
        + "    ) " + nl
        + "select" + nl
        + "    {" + nl
        + "        [Measures].[Transaction Count]," + nl
        + "        [Measures].[Transaction Amount]," + nl
        + "        [Measures].[m]" + nl
        + "    } on columns," + nl
        + "    Order(" + nl
        ...
        "select" + nl
        + " {[Measures].[Transaction Count],[Measures].[Transaction Amount]} on columns," + nl
        + " [Time].[2014].[1].[3].[9].[1].CHILDREN on rows" + nl
        + "from [ANZ Transaction]",
        "select" + nl
        + " {[Measures].[Transaction Count],[Measures].[Transaction Amount]} on columns," + nl
        + " [Time].MEMBERS on rows" + nl
        + "from [ANZ Transaction]",
        "select" + nl
        + " {[Measures].[Transaction Count],[Measures].[Transaction Amount]} on columns," + nl
        + " [Time].[2014].[3].[1].[13]:[Time].[2014].[3].[1].[15] on rows" + nl
        + "from [ANZ Transaction]",
        "select" + nl
        + " {[Measures].[Transaction Count],[Measures].[Transaction Amount]} on columns," + nl
        + " [Time.ByQuarter].[2014].[1].[3].[1].[13]:[Time.ByQuarter].[2014].[1].[3].[1].[15] on rows" + nl
        + "from [ANZ Transaction]",
        "select" + nl
        + " {[Measures].[Transaction Count],[Measures].[Transaction Amount]} on columns," + nl
        + " [Time.ByWeek].[2014].[9].[6].[13]:[Time.ByWeek].[2014].[9].[6].[15] on rows" + nl
        + "from [ANZ Transaction]",};
%>
...
<html>
    ...
    <body>
        ...
        <a href="anz_olap.html">ANZ Simple OLAP</a><p/>
